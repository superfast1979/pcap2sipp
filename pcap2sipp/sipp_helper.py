...
def writeScenarioHeader(path, file):
    with open(os.path.join(path,file), "wb") as scenario:
        scenario.write('<?xml version="1.0" encoding="ISO-8859-1"?>\n')
        scenario.write('<scenario name="{}">\n'.format(file))
        
def writeScenarioFooter(path, file):
    with open(os.path.join(path, file), "a+b") as scenario:
        scenario.write('  <ResponseTimeRepartition value="10, 20, 30, 40, 50, 100, 150, 200"/>\n')
        scenario.write('  <CallLengthRepartition value="10, 50, 100, 500, 1000, 5000, 10000"/>\n')
